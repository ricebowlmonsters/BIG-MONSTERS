(function () {
  ...
  function renderTargetSettings() {
    var html = '<table class="bep-target-table"><thead><tr><th>Outlet</th><th>Real</th><th>Target</th><th>Simulasi</th></tr></thead><tbody>';
    outlets.forEach(function (outlet) { var realBep = window.RbmBepCalculations.calculateRealBep(outlet); var target = outlet.targetAuto !== false ? realBep : outlet.targetRevenue; if (outlet.targetAuto !== false) outlet.targetRevenue = target; html += '<tr><td>' + escapeHtml(outlet.name) + '<small class="target-source">' + (outlet.targetAuto !== false ? 'Otomatis dari BEP Real' : 'Manual') + '</small></td><td>' + money(outlet.revenue) + '</td><td><input type="number" min="0" data-target-input="' + outlet.id + '" value="' + Math.round(target) + '"><button type="button" class="target-reset" data-target-reset="' + outlet.id + '">Auto</button></td><td><input type="number" min="0" data-simulation-setting="' + outlet.id + '" value="' + outlet.simulationRevenue + '"></td></tr>'; });
